...
interface HomeProps {
  onSelect: (mode: 'interview' | 'article') => void;
}
...
export default function Home({ onSelect }: HomeProps) {
  ...
  useEffect(() => {
    async function init() {
      try {
        const status = await (window as any).electron.initCopilot();
        setServiceStatus(status === 'ready' ? 'ready' : 'error');
      } catch (e) {
        console.error(e);
      ...
      <div className="w-full max-w-6xl">
        <div className="mb-12 text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">
            ...
            <a
              ...
              onClick={(e) => {
                e.preventDefault();
                (window as any).electron.openExternal('https://github.com/estruyf/ghostwriter-agents-ai');
              }}
              className="text-purple-300 hover:text-purple-200 transition underline cursor-pointer"
            >
              Ghostwriter Agents
            </a>
          </p>
        ...
        {serviceStatus === 'initializing' && (
          <div className="flex flex-col items-center justify-center space-y-4 py-12">
            ...
            <p className="text-slate-400">Starting GitHub Copilot Service...</p>
          </div>
        )}

        {serviceStatus === 'error' && (
          <div className="text-center py-12">
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-red-500/20 text-3xl">⚠️</div>
            <h3 className="mt-4 text-xl font-bold text-red-400">Service Failed to Start</h3>
            <p className="mt-2 text-slate-400">Could not connect to GitHub Copilot CLI.</p>
          </div>
        )}
        ...
        <div className="mt-8 text-center text-sm text-slate-400">
          Powered by GitHub Copilot CLI & SDK
          <br />
          <span className="text-xs">
            ...
            <a
              ...
              onClick={(e) => {
                e.preventDefault();
                (window as any).electron.openExternal('https://www.eliostruyf.com');
              }}
              className="text-purple-300 hover:text-purple-200 transition underline cursor-pointer"
            >
              Elio Struyf
